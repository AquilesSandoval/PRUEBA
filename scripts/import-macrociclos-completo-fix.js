...
(async () => {
  try {
    ...
    for (const macro of macros) {
      ...
      for (let semanaIndex = 0; semanaIndex < (macro.semanas || []).length; semanaIndex++) {
        ...
        for (const dia of (semana.dias || [])) {
          ...
          for (const ejercicio of (dia.ejercicios || [])) {
            ...
            const hora = ejercicio.hora || '00:00';
            ...
            // Insertar sesión con TODOS los datos - INSERT INDIVIDUAL para asegurar que se guarde
            try {
              await pool.query(
              ...
              console.error(`Error al insertar sesión: ${err.message}`);
            }
          }
        }
      }
      ...
      console.log(`   └─ ${macro.semanas?.length || 0} semanas → ${sesionesCreadas} sesiones guardadas`);
    ...
    const verificacion = await pool.query(`
      SELECT 
        (SELECT COUNT(*) FROM macrociclos) as total_macros,
        (SELECT COUNT(*) FROM sesiones WHERE macrociclo_id IS NOT NULL) as total_sesiones
    `);
    
    console.log('\n=== VERIFICACIÓN EN BD ===');
    console.log(`📊 Macrociclos en BD: ${verificacion.rows[0].total_macros}`);
    console.log(`📊 Sesiones en BD: ${verificacion.rows[0].total_sesiones}`);
