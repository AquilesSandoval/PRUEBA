...
// Get microciclos with pagination
app.get('/api/microciclos', async (req, res) => {
  try {
    const { atleta_id, page = 1, limit = 100 } = req.query;
    const offset = (page - 1) * limit;
    
    let countQuery = `SELECT COUNT(*) as total FROM microciclos m WHERE m.mesociclo_id IS NULL`;
    let query = `
      SELECT 
        m.*,
        a.nombre as atleta_nombre,
        a.apellido as atleta_apellido,
        (SELECT COUNT(*) FROM sesiones s WHERE s.microciclo_id = m.id) as total_sesiones
      FROM microciclos m
      LEFT JOIN atletas a ON m.atleta_id = a.id
      WHERE m.mesociclo_id IS NULL
    `;
    ...
    if (atleta_id) {
      query += ' AND m.atleta_id = $1';
      countQuery += ' AND m.atleta_id = $1';
      params.push(atleta_id);
      countParams.push(atleta_id);
    }
...
app.get('/api/microciclos/:id', async (req, res) => {
  try {
    ...
    diasSemana.forEach(dia => {
      dias[dia] = { nombre: dia, sesiones: [] };
    });
    
    // Llenar con sesiones
    sesionesResult.rows.forEach(sesion => {
      const diaNombre = sesion.dia_nombre || 'Lunes';
      if (dias[diaNombre]) {
